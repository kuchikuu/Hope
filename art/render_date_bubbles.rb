...
images = {
    'date_bubble_black.svg' => ['date_bubble_black.9', 0],
	}
...
#		next if left.length != 2
		...
		top_line = draw_format % [12 * factor, 0, 16 * factor, 0]
		right_line = draw_format % [width + 1, 6 * factor, width + 1, 22 * factor]
		bottom_line = draw_format % [10 * factor, height + 1, 18 * factor, height + 1]
		left_line = draw_format % [0, 9 * factor, 0, 11 * factor]
